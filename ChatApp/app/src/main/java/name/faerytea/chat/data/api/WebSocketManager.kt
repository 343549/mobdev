...
class WebSocketManager(private val client: OkHttpClient) {
    ...
    fun connect(username: String, token: String) {
        ...
        webSocket?.close(1000, null)  // закрываем предыдущее если было
        ...
        webSocket = client.newWebSocket(request, object : WebSocketListener() {
            override fun onMessage(webSocket: WebSocket, text: String) {
                try {
                    ...
                    // Парсим Message вручную т.к. MessageData требует кастомного сериализатора
                    ...
                    // игнорируем нераспознанные события (TypingChanged и т.д.)
                }
            }

            override fun onFailure(webSocket: WebSocket, t: Throwable, response: Response?) {
                // соединение упало — переподключимся при следующей необходимости
            }
        })
    }
